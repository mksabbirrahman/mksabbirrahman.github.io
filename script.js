...
// --- Content helpers for Posts & Quotes (robust path detection) ---
...
function postsDataPath() {
  // If page is inside /posts/ directory, read from relative `data/posts.json`.
  // Otherwise, read from root `posts/data/posts.json`.
  const p = location.pathname || '';
  if (p.startsWith('/posts/') || p === '/posts' || p.endsWith('/posts') ) return 'data/posts.json';
  return 'posts/data/posts.json';
}

function quotesDataPath() {
  // If page is inside /posts/ directory, the quotes file is one level up
  const p = location.pathname || '';
  if (p.startsWith('/posts/') || p === '/posts' || p.endsWith('/posts')) return '../data/quotes.json';
  return 'data/quotes.json';
}

async function initPostsPage() {
  const path = postsDataPath();
...
function renderPostsList(root, posts) {
  ...
  posts.forEach(p => {
    ...
    const linkHref = (location.pathname.startsWith('/posts/') || location.pathname === '/posts') ? `?slug=${encodeURIComponent(p.slug)}` : `posts/?slug=${encodeURIComponent(p.slug)}`;
    const title = createEl('h2', {}, `<a href="${linkHref}">${p.title}</a>`);
...
function renderPostBySlug(root, posts, slug) {
  ...
  const backHref = (location.pathname.startsWith('/posts/') || location.pathname === '/posts') ? './' : 'posts/';
  const back = createEl('p', {}, `<a href="${backHref}">← Back to posts</a>`);
